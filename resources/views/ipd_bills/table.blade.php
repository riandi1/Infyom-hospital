    ...
    <div class="col-lg-6 col-md-12">
        ...
        <form id="ipdBillForm">
            ...
            <a href="{{ url('ipd-bills/'.$ipdPatientDepartment->id.'/pdf') }}" target="_blank"
               class="btn btn-secondary mx-2 mb-3 font-weight-bold {{ ($ipdPatientDepartment->bill) ? '' : 'disabled' }} "
               id="printBillBtn" role="button" aria-pressed="true">{{ __('messages.bill.print_bill')  }}</a>
            @if(!$ipdPatientDepartment->bill_status)
                <button type="submit" class="btn btn-secondary mx-2 mb-3 font-weight-bold" id="saveIpdBillbtn">
